...
#include "CppRestOpenAPIClient/model/Balance.h"
...
bool Balance::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(U("data"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            std::vector<std::shared_ptr<Balance_data_inner>> refVal_setData;
...
std::vector<std::shared_ptr<Balance_data_inner>>& Balance::getData()
{
    return m_Data;
}

void Balance::setData(const std::vector<std::shared_ptr<Balance_data_inner>>& value)
{
    m_Data = value;
    m_DataIsSet = true;
}
