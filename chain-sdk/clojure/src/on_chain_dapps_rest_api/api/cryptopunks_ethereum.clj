(ns on-chain-dapps-rest-api.api.cryptopunks-ethereum
...
(defn-spec c-ryptopunksethereum-bids-current-with-http-info any?
  "Bids (current)
  Gets bids."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/bids/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-bids-current (s/coll-of cryptopunks/bid-dto-spec)
  "Bids (current)
  Gets bids."
  []
  (let [res (:data (c-ryptopunksethereum-bids-current-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of cryptopunks/bid-dto-spec) res st/string-transformer)
       res)))


(defn-spec c-ryptopunksethereum-collection-daily-snapshots-current-with-http-info any?
  "CollectionDailySnapshots (current)
  Gets collectionDailySnapshots."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/collectionDailySnapshots/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-collection-daily-snapshots-current (s/coll-of cryptopunks/collection-daily-snapshot-dto-spec)
  "CollectionDailySnapshots (current)
  Gets collectionDailySnapshots."
  []
  (let [res (:data (c-ryptopunksethereum-collection-daily-snapshots-current-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of cryptopunks/collection-daily-snapshot-dto-spec) res st/string-transformer)
       res)))


(defn-spec c-ryptopunksethereum-collections-current-with-http-info any?
  "Collections (current)
  Gets collections."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/collections/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-collections-current (s/coll-of cryptopunks/collection-dto-spec)
  "Collections (current)
  Gets collections."
  []
  (let [res (:data (c-ryptopunksethereum-collections-current-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of cryptopunks/collection-dto-spec) res st/string-transformer)
       res)))


(defn-spec c-ryptopunksethereum-data-sources-current-with-http-info any?
  "DataSources (current)
  Gets dataSources."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/dataSources/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-data-sources-current (s/coll-of cryptopunks/data-sources-dto-spec)
  "DataSources (current)
  Gets dataSources."
  []
  (let [res (:data (c-ryptopunksethereum-data-sources-current-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of cryptopunks/data-sources-dto-spec) res st/string-transformer)
       res)))


(defn-spec c-ryptopunksethereum-items-current-with-http-info any?
  "Items (current)
  Gets items."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/items/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-items-current (s/coll-of cryptopunks/item-dto-spec)
  "Items (current)
  Gets items."
  []
  (let [res (:data (c-ryptopunksethereum-items-current-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of cryptopunks/item-dto-spec) res st/string-transformer)
       res)))


(defn-spec c-ryptopunksethereum-market-places-current-with-http-info any?
  "MarketPlaces (current)
  Gets marketPlaces."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/marketPlaces/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-market-places-current (s/coll-of cryptopunks/market-place-dto-spec)
  "MarketPlaces (current)
  Gets marketPlaces."
  []
  (let [res (:data (c-ryptopunksethereum-market-places-current-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of cryptopunks/market-place-dto-spec) res st/string-transformer)
       res)))


(defn-spec c-ryptopunksethereum-marketplace-daily-snapshots-current-with-http-info any?
  "MarketplaceDailySnapshots (current)
  Gets marketplaceDailySnapshots."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/marketplaceDailySnapshots/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-marketplace-daily-snapshots-current (s/coll-of cryptopunks/marketplace-daily-snapshot-dto-spec)
  "MarketplaceDailySnapshots (current)
  Gets marketplaceDailySnapshots."
  []
  (let [res (:data (c-ryptopunksethereum-marketplace-daily-snapshots-current-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of cryptopunks/marketplace-daily-snapshot-dto-spec) res st/string-transformer)
       res)))


(defn-spec c-ryptopunksethereum-trades-current-with-http-info any?
  "Trades (current)
  Gets trades."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/trades/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-trades-current (s/coll-of cryptopunks/trade-dto-spec)
  "Trades (current)
  Gets trades."
  []
  (let [res (:data (c-ryptopunksethereum-trades-current-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode (s/coll-of cryptopunks/trade-dto-spec) res st/string-transformer)
       res)))


(defn-spec c-ryptopunksethereum-users-current-with-http-info any?
  "Users (current)
  Gets users."
  []
  (call-api "/v1/dapps/cryptopunks-ethereum/users/current" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec c-ryptopunksethereum-users-current (s/coll-of cryptopunks/user-dto-spec)
  "Users (current)
  Gets users."
  []
  (let [res (:data (c-ryptopunksethereum-users-current-with-http-info))]
    (if (:decode-models *api-context*)
