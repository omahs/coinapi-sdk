  ...
  class UniswapV3PoolDayDataDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UniswapV3PoolDayDataDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UniswapV3PoolDayDataDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [entry_time, recv_time, block_number, id, date, pool, liquidity, sqrt_price, token_0_price, token_1_price, tick, fee_growth_global_0x128, fee_growth_global_1x128, tvl_usd, volume_token_0, volume_token_1, volume_usd, fees_usd, tx_count, open, high, low, close, vid].hash
    end
