  ...
  class UniswapV2TransactionDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UniswapV2TransactionDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UniswapV2TransactionDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'mints')
        if (value = attributes[:'mints']).is_a?(Array)
          self.mints = value
        end
      end

      if attributes.key?(:'burns')
        if (value = attributes[:'burns']).is_a?(Array)
          self.burns = value
        end
      end

      if attributes.key?(:'swaps')
        if (value = attributes[:'swaps']).is_a?(Array)
          self.swaps = value
        end
      end
    ...
    def hash
      [entry_time, recv_time, block_number, id, timestamp, mints, burns, swaps, vid].hash
    end
