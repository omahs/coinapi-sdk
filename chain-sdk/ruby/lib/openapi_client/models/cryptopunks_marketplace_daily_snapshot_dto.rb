...
module OpenapiClient
  class CRYPTOPUNKSMarketplaceDailySnapshotDTO
    ...
    # 
    attr_accessor :daily_active_traders

    # 
    attr_accessor :daily_traded_collection_count

    # 
    attr_accessor :daily_traded_item_count
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CRYPTOPUNKSMarketplaceDailySnapshotDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CRYPTOPUNKSMarketplaceDailySnapshotDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'daily_active_traders')
        self.daily_active_traders = attributes[:'daily_active_traders']
      end

      if attributes.key?(:'daily_traded_collection_count')
        self.daily_traded_collection_count = attributes[:'daily_traded_collection_count']
      end

      if attributes.key?(:'daily_traded_item_count')
        self.daily_traded_item_count = attributes[:'daily_traded_item_count']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          daily_active_traders == o.daily_active_traders &&
          daily_traded_collection_count == o.daily_traded_collection_count &&
          daily_traded_item_count == o.daily_traded_item_count
    end
    ...
    def hash
      [entry_time, recv_time, block_number, vid, block_range, id, marketplace, timestamp, collection_count, cumulative_trade_volume_eth, marketplace_revenue_eth, creator_revenue_eth, total_revenue_eth, trade_count, cumulative_unique_traders, daily_active_traders, daily_traded_collection_count, daily_traded_item_count].hash
    end
