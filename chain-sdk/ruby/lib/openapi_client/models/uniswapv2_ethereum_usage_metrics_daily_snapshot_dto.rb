  ...
  class UNISWAPV2ETHEREUMUsageMetricsDailySnapshotDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UNISWAPV2ETHEREUMUsageMetricsDailySnapshotDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UNISWAPV2ETHEREUMUsageMetricsDailySnapshotDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [entry_time, recv_time, block_number, block_range, id, protocol, daily_active_users, cumulative_unique_users, daily_transaction_count, daily_deposit_count, daily_withdraw_count, daily_swap_count, total_pool_count, timestamp].hash
    end
