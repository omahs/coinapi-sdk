  ...
  class CRYPTOPUNKSBidDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CRYPTOPUNKSBidDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CRYPTOPUNKSBidDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tokens_bid')
        self.tokens_bid = attributes[:'tokens_bid']
      end
    ...
    def hash
      [entry_time, recv_time, block_number, vid, block_range, id, tokens_bid, token_id, timestamp, bidder].hash
    end
