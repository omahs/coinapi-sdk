import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for DexWithdrawRequestDTO
void main() {
  final instance = DexWithdrawRequestDTOBuilder();
  // TODO add properties to the builder and call build()

  group(DexWithdrawRequestDTO, () {
    // DateTime entryTime
    test('to test the property `entryTime`', () async {
      // TODO
    });

    // DateTime recvTime
    test('to test the property `recvTime`', () async {
      // TODO
    });

    // Number of block in which entity was recorded.
    // int blockNumber
    test('to test the property `blockNumber`', () async {
      // TODO
    });

    // Identifier, format: (transaction hash)-(id).
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // 
    // String user
    test('to test the property `user`', () async {
      // TODO
    });

    // 
    // String tokenAddress
    test('to test the property `tokenAddress`', () async {
      // TODO
    });

    // 
    // String amount
    test('to test the property `amount`', () async {
      // TODO
    });

    // 
    // String withdrawableFromBatchId
    test('to test the property `withdrawableFromBatchId`', () async {
      // TODO
    });

    // 
    // String createEpoch
    test('to test the property `createEpoch`', () async {
      // TODO
    });

    // 
    // String createBatchId
    test('to test the property `createBatchId`', () async {
      // TODO
    });

    // 
    // String txHash
    test('to test the property `txHash`', () async {
      // TODO
    });

    // 
    // int vid
    test('to test the property `vid`', () async {
      // TODO
    });

  });
}
