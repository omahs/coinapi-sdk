...
class UniswapV2PairDTO {
  /// Returns a new [UniswapV2PairDTO] instance.
  UniswapV2PairDTO({
    this.entryTime,
    this.recvTime,
    this.blockNumber,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UniswapV2PairDTO &&
  ...
  /// Returns a new [UniswapV2PairDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static UniswapV2PairDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UniswapV2PairDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UniswapV2PairDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UniswapV2PairDTO> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UniswapV2PairDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, UniswapV2PairDTO> mapFromJson(dynamic json) {
    final map = <String, UniswapV2PairDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UniswapV2PairDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of UniswapV2PairDTO-objects as value to a dart map
