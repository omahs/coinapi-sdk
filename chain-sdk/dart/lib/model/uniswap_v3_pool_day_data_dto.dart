...
class UniswapV3PoolDayDataDTO {
  /// Returns a new [UniswapV3PoolDayDataDTO] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UniswapV3PoolDayDataDTO &&
  ...
  @override
  String toString() => 'UniswapV3PoolDayDataDTO[entryTime=$entryTime, recvTime=$recvTime, blockNumber=$blockNumber, id=$id, date=$date, pool=$pool, liquidity=$liquidity, sqrtPrice=$sqrtPrice, token0Price=$token0Price, token1Price=$token1Price, tick=$tick, feeGrowthGlobal0x128=$feeGrowthGlobal0x128, feeGrowthGlobal1x128=$feeGrowthGlobal1x128, tvlUsd=$tvlUsd, volumeToken0=$volumeToken0, volumeToken1=$volumeToken1, volumeUsd=$volumeUsd, feesUsd=$feesUsd, txCount=$txCount, open=$open, high=$high, low=$low, close=$close, vid=$vid]';
  ...
  /// Returns a new [UniswapV3PoolDayDataDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static UniswapV3PoolDayDataDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UniswapV3PoolDayDataDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UniswapV3PoolDayDataDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UniswapV3PoolDayDataDTO> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <UniswapV3PoolDayDataDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UniswapV3PoolDayDataDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, UniswapV3PoolDayDataDTO> mapFromJson(dynamic json) {
    final map = <String, UniswapV3PoolDayDataDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UniswapV3PoolDayDataDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of UniswapV3PoolDayDataDTO-objects as value to a dart map
  static Map<String, List<UniswapV3PoolDayDataDTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UniswapV3PoolDayDataDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = UniswapV3PoolDayDataDTO.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
