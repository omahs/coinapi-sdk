...
class CowSettlementDTO {
  /// Returns a new [CowSettlementDTO] instance.
  CowSettlementDTO({
    this.entryTime,
    this.recvTime,
    this.blockNumber,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CowSettlementDTO &&
  ...
  @override
  String toString() => 'CowSettlementDTO[entryTime=$entryTime, recvTime=$recvTime, blockNumber=$blockNumber, id=$id, solver=$solver, txHash=$txHash, firstTradeTimestamp=$firstTradeTimestamp, vid=$vid]';
  ...
  /// Returns a new [CowSettlementDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CowSettlementDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CowSettlementDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CowSettlementDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CowSettlementDTO> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CowSettlementDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CowSettlementDTO> mapFromJson(dynamic json) {
    final map = <String, CowSettlementDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CowSettlementDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CowSettlementDTO-objects as value to a dart map
