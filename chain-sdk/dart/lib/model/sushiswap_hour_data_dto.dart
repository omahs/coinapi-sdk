...
class SushiswapHourDataDTO {
  /// Returns a new [SushiswapHourDataDTO] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SushiswapHourDataDTO &&
  ...
  @override
  String toString() => 'SushiswapHourDataDTO[entryTime=$entryTime, recvTime=$recvTime, blockNumber=$blockNumber, id=$id, date=$date, factory_=$factory_, volumeEth=$volumeEth, volumeUsd=$volumeUsd, untrackedVolume=$untrackedVolume, liquidityEth=$liquidityEth, liquidityUsd=$liquidityUsd, txCount=$txCount, vid=$vid]';
  ...
  /// Returns a new [SushiswapHourDataDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SushiswapHourDataDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SushiswapHourDataDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SushiswapHourDataDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SushiswapHourDataDTO> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SushiswapHourDataDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SushiswapHourDataDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SushiswapHourDataDTO> mapFromJson(dynamic json) {
    final map = <String, SushiswapHourDataDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SushiswapHourDataDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SushiswapHourDataDTO-objects as value to a dart map
  static Map<String, List<SushiswapHourDataDTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SushiswapHourDataDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SushiswapHourDataDTO.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
