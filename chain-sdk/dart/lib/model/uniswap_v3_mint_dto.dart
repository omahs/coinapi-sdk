...
class UniswapV3MintDTO {
  /// Returns a new [UniswapV3MintDTO] instance.
  UniswapV3MintDTO({
    this.entryTime,
    this.recvTime,
    this.blockNumber,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UniswapV3MintDTO &&
  ...
  /// Returns a new [UniswapV3MintDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static UniswapV3MintDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UniswapV3MintDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UniswapV3MintDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UniswapV3MintDTO> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <UniswapV3MintDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UniswapV3MintDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, UniswapV3MintDTO> mapFromJson(dynamic json) {
    final map = <String, UniswapV3MintDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UniswapV3MintDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of UniswapV3MintDTO-objects as value to a dart map
  static Map<String, List<UniswapV3MintDTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UniswapV3MintDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = UniswapV3MintDTO.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
