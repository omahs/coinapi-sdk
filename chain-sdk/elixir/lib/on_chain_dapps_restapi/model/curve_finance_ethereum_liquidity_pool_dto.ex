# NOTE: This file is auto generated by OpenAPI Generator 6.6.0 (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule OnChainDapps-RESTAPI.Model.CurveFinanceEthereumLiquidityPoolDto do
  @moduledoc """
  
  """

  @derive [Poison.Encoder]
  defstruct [
    :entry_time,
    :recv_time,
    :block_number,
    :vid,
    :id,
    :protocol,
    :name,
    :symbol,
    :input_tokens,
    :input_tokens_ordered,
    :output_token,
    :reward_tokens,
    :fees,
    :is_single_sided,
    :created_timestamp,
    :created_block_number,
    :total_value_locked_usd,
    :cumulative_supply_side_revenue_usd,
    :cumulative_protocol_side_revenue_usd,
    :cumulative_total_revenue_usd,
    :cumulative_volume_usd,
    :input_token_balances,
    :input_token_weights,
    :output_token_supply,
    :output_token_price_usd,
    :staked_output_token_amount,
    :reward_token_emissions_amount,
    :reward_token_emissions_usd,
    :registry_address,
    :gauge_address,
    :evaluated_ask
  ]

  @type t :: %__MODULE__{
    :entry_time => DateTime.t | nil,
    :recv_time => DateTime.t | nil,
    :block_number => integer() | nil,
    :vid => integer() | nil,
    :id => String.t | nil,
    :protocol => String.t | nil,
    :name => String.t | nil,
    :symbol => String.t | nil,
    :input_tokens => [String.t] | nil,
    :input_tokens_ordered => [String.t] | nil,
    :output_token => String.t | nil,
    :reward_tokens => [String.t] | nil,
    :fees => [String.t] | nil,
    :is_single_sided => boolean() | nil,
    :created_timestamp => String.t | nil,
    :created_block_number => String.t | nil,
    :total_value_locked_usd => String.t | nil,
    :cumulative_supply_side_revenue_usd => String.t | nil,
    :cumulative_protocol_side_revenue_usd => String.t | nil,
    :cumulative_total_revenue_usd => String.t | nil,
    :cumulative_volume_usd => String.t | nil,
    :input_token_balances => [String.t] | nil,
    :input_token_weights => [String.t] | nil,
    :output_token_supply => String.t | nil,
    :output_token_price_usd => String.t | nil,
    :staked_output_token_amount => String.t | nil,
    :reward_token_emissions_amount => [String.t] | nil,
    :reward_token_emissions_usd => [String.t] | nil,
    :registry_address => String.t | nil,
    :gauge_address => String.t | nil,
    :evaluated_ask => float() | nil
  }
end

defimpl Poison.Decoder, for: OnChainDapps-RESTAPI.Model.CurveFinanceEthereumLiquidityPoolDto do
  def decode(value, _options) do
    value
  end
end

